...
TEMPLATE_FILE = os.path.join(BASE_DIR, "매직_견적서_cs_은혜님.xlsx")
...
def create_fitted_excel_image(
    ...
    """
    D:E 병합셀 영역에 최대한 크게 맞춤.
    비율은 유지하고 셀 안에서 가운데 정렬.
    """
...
def build_quote(urls: List[str]) -> tuple[bytes, str]:
    if not os.path.exists(TEMPLATE_FILE):
        raise FileNotFoundError("앱과 같은 폴더에 매직_견적서_cs_은혜님.xlsx 파일이 있어야 합니다.")
...
def main():
    ...
    st.title("매직 견적서 생성기")
    st.caption("15행부터 상품이 추가되고, D:E 병합영역에 이미지가 셀 맞춤으로 배치됩니다.")

    st.session_state.setdefault("logs", [])

    with st.sidebar:
        st.subheader("기준 양식")
        st.write("템플릿 파일: 매직_견적서_cs_은혜님.xlsx")
        st.write("비밀번호: 0915")
